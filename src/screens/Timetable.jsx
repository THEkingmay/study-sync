...
export default function Timetable() {
  ...
  const isValidDate = (dateStr) => {
    ...
    if (year < 2500 || year > 2600) return false; // ปี พ.ศ. ปรับได้
  ...
  const handleSave = () => {
    ...
    if (activeTab === "study") {
      const startMin = convertToMinutes(form.start);
      const endMin = convertToMinutes(form.end);
    ...
    if (activeTab === "exam") {
      if (!form.examDate || !form.examTime) {
        alert("กรุณากรอกวันและเวลาสอบ");
        return;
      }

      if (!isValidDate(form.examDate)) {
        alert("รูปแบบวันที่ต้องเป็น dd/mm/yyyy และอยู่ในช่วงที่ถูกต้อง");
        return;
      }

      const parts = form.examTime.split("-");
      if (parts.length !== 2) {
        alert("รูปแบบเวลาต้องเป็น เช่น 09:00-12:00");
        return;
      }
      ...
      if (startMin >= endMin) {
        alert("เวลาเริ่มสอบต้องน้อยกว่าเวลาสิ้นสุด");
        return;
      }
    }
    const newItem = {
          ...
          {subjects.length === 0 ? (
            ...
            subjects.map((item) => (
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => handleDelete(item.id, "study")}
      ...
      {data.length === 0 ? (
        <View style={styles.emptyExamCard}>
          ...
          <Text style={styles.examSub}>
            คลิกปุ่ม "+ เพิ่มข้อมูล" ด้านบนเพื่อเริ่มต้น
          </Text>
        </View>
      ) : (
        data.map((item) => (
          ...
          >
            <View
              style={[
                styles.examTypeTag,
                item.examType === "mid" ? styles.midTag : styles.finalTag,
              ]}
            >
              <Text style={styles.tagText}>
                {item.examType === "mid" ? "Midterm" : "Final"}
              </Text>
            </View>
            <Text style={styles.examCourseCode}>
              {item.code} {item.name}
            </Text>
            ...
            <TouchableOpacity
              onPress={() => handleDelete(item.id, "exam")}
